...
public class KouhoDelete {
	public ArrayList<String> kouhoDelete(ArrayList<String> list) {
		...
		do {
			...
			//横方向へ他のマスに入っていない数字があれば残す(iは縦のループ)(jは横のループ)
			for(int i = 0; i < 9; i++) {
				for(int j = 0; j < 9; j++) {
					int karinum = i * 9 + 1 + j;
					if(!(map.get(karinum) == null)) {
						ArrayList<Integer> lists = new ArrayList<Integer>();
						String[] split = map.get(karinum).split(",");
						for(int l = 0; l < split.length; l++) {
							lists.add(Integer.parseInt(split[l]));
						}
						for(int k = 0; k < lists.size(); k++) {
							for(int d = 0; d < 9; d++) {
								int kouhoNum = i * 9 + 1 + d;
								if(karinum != kouhoNum && map.get(kouhoNum) != null) {
									String line = map.get(kouhoNum);
									String[] splits = line.split(",");
									int[] kariKouho = new int[splits.length];
									for(int r = 0; r < splits.length; r++) {
										kariKouho[r] = Integer.parseInt(splits[r]);
									}
									for(int y = 0; y < kariKouho.length; y++) {
										for(int s = 0; s < lists.size(); s++) {
											if(lists.get(s) == kariKouho[y]) {
												lists.remove(s);
											}
										}
									}
			...
			//縦方向へ他のマスに入っていない数字があれば残す(iは縦のループ)(jは横のループ)
			for(int j = 0; j < 9; j++) {
				for(int i = 0; i < 9; i++) {
					int karinum = i * 9 + 1 + j;
					if(!(map.get(karinum) == null)) {
